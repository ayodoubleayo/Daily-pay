...
export default function AllSellersPage() {
  ...
  /**
   * Fetches the list of sellers, including the necessary admin secret.
  ...
  const loadSellers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/admin/active-sellers', {
        headers: {
          'x-admin-secret': secret, // CRITICAL: Send the admin secret
      ...
      if (!res.ok) {
        ...
        showAlert(errorData.error || 'Failed to fetch sellers data. Authorization denied.', 'error');
        setSellers([]);
        return;
      }
  ...
  const warnSeller = useCallback((id) => {
    const confirmMsg = 'Are you sure you want to add a warning to this seller?';
    confirmThen(`/api/admin/sellers/${id}/warn`, 'PUT', null, confirmMsg);
  }, [confirmThen]);
  
  // Helper for stats
  const getSellerStats = (sellers) => ({
    total: sellers.length,
    approved: sellers.filter(s => s.approved === true).length,
    pending: sellers.filter(s => s.approved === false).length,
    suspendedOrBanned: sellers.filter(s => s.status === "banned" || s.status === "suspended").length,
  });

  const stats = getSellerStats(sellers);

  return (
    <div className="max-w-4xl mx-auto p-4">
      <h1 className="text-3xl font-extrabold mb-6 text-gray-800 border-b pb-2">All Sellers Management</h1>
      ...
      {loading && <div className="p-4 text-center text-blue-500">Loading sellers…</div>}

      {!loading && sellers.length === 0 && (
         <div className="p-4 bg-yellow-100 text-yellow-800 rounded">No sellers found or unauthorized access.</div>
      )}

      <div className="grid gap-3">
        {sellers.map(s => (
          <div key={s._id} className="p-3 bg-white shadow rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              <p className="font-semibold text-gray-900">Shop: {s.shopName || s.name || 'N/A'}</p>
            ...
            <div className="flex flex-wrap justify-end gap-2 mt-3 md:mt-0">
              ...
              {!s.banned && !s.suspended && (
                <button 
                  onClick={() => confirmThen(`/api/admin/sellers/${s._id}/suspend`, 'PUT', null, 'Suspend this seller? They will not be able to sell products.')} 
              ...
              {!s.banned && s.suspended && (
                <button 
                  onClick={() => confirmThen(`/api/admin/sellers/${s._id}/reactivate`, 'PUT', null, 'Unsuspend this seller? They will regain selling access.')} 
              ...
              {!s.banned && (
                <button 
                  onClick={() => confirmThen(`/api/admin/sellers/${s._id}/ban`, 'PUT', null, 'WARNING: Permanently ban this seller? This action is difficult to reverse.')} 
              ...
              {s.banned && (
                <button 
                  onClick={() => confirmThen(`/api/admin/sellers/${s._id}/unban`, 'PUT', null, 'Unban this seller?')} 
