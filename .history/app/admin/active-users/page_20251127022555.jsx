...
import { Users, Clock, Zap, XCircle } from 'lucide-react';
...
export default function ActiveUsersPage() {
  ...
  /**
   * Fetches the list of active users, ensuring the admin secret header is included.
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showAlert(errorData.error || 'Failed to fetch users. Authorization denied.', 'error');
        setUsers([]);
        return;
      }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="p-4 bg-blue-50 border border-blue-200 text-blue-800 rounded-lg mb-6 flex items-center">
        ...
        <p className="text-sm">
          This panel shows users (non-sellers) who have recently interacted with the platform.
        </p>
      </div>
      ...
      {!loading && users.length === 0 && message?.type !== 'error' && (
         <div className="p-6 bg-yellow-100 text-yellow-800 rounded-lg flex items-center">
            ...
            No general users detected as active currently.
        </div>
      )}

      <div className="grid gap-3">
        {users.map(u => (
          <div key={u._id} className="p-4 bg-white shadow-lg rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center border-l-4 border-red-600">
            <div>
            ...
            <div className="mt-2 sm:mt-0 flex items-center text-sm text-gray-700">
                ...
                <p>Last Active: <span className="font-medium">{new Date(u.lastActive).toLocaleString()}</span></p>
            </div>
          </div>
        ))}
      </div>
